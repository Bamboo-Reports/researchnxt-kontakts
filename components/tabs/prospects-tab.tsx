...
export function ProspectsTab({
  ...
  const sortedProspects = useMemo(() => {
    ...
    const getValue = (prospect: Prospect): string | null => {
      switch (sort.key) {
        case "name":
  ...
  const getSortIcon = (sortKey: typeof sort.key): JSX.Element => {
    if (sort.key !== sortKey || sort.direction === null) {
      return <ArrowUpDown className="h-3.5 w-3.5 text-muted-foreground" />
    }
    if (sort.direction === "asc") {
      return <ArrowUpAZ className="h-3.5 w-3.5 text-muted-foreground" />
    }
    return <ArrowDownAZ className="h-3.5 w-3.5 text-muted-foreground" />
  }
  ...
  const pageInfo = getPageInfo(currentPage, filteredCount, itemsPerPage)
  const totalPages = getTotalPages(filteredCount, itemsPerPage)
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-6 py-4 border-t shrink-0 bg-muted/20">
              <div className="flex flex-wrap items-center gap-3">
                <p className="text-sm text-muted-foreground">
                  {pageInfo.startItem}-{pageInfo.endItem} of {filteredCount}
                </p>
              </div>
              {totalPages > 1 && (
